...
**Kyverno policy enforcement as a second enforcement layer.** The PR analysis agent (Section 6.5) performs semantic drift detection at review time, comparing pull request diffs against the authoritative `docs/deployment-policy.md` to surface violations before merge. This is a soft gate: the agent advises, but a human must act on it. Kyverno would provide the hard gate at the other end of the pipeline — an admission controller that evaluates the same policy rules at apply time, independently of whether the PR analysis ran or was heeded.

The policies documented in `sample-backend-gitops/docs/deployment-policy.md` translate directly into Kyverno `ClusterPolicy` resources: minimum replica count, mandatory liveness and readiness probes, RollingUpdate strategy with `maxUnavailable: 0`, explicit resource requests and limits, image registry restriction to the internal registry, and prohibition of mutable tags such as `latest`. The drift demo (Section 6.5) explicitly demonstrated that a PR can simultaneously violate all six of these policies while appearing syntactically valid and passing CI tests. Kyverno would ensure that even if such a PR were merged, the resulting manifest would be rejected at the cluster boundary before any pod were scheduled.

The two layers are complementary rather than redundant: the PR analysis agent catches violations early in the developer loop, when they are cheapest to fix, and provides natural-language reasoning about why a change is problematic. Kyverno enforces the same constraints deterministically at deploy time, removing the dependency on human action between detection and prevention. Together they implement a defence-in-depth model — early warning at the PR stage, hard enforcement at the cluster boundary — without any single point of failure.
